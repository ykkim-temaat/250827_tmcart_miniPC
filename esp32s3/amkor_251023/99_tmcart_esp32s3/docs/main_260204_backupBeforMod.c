...
static const char *FIRMWARE_VERSION = "TMCart_ESP32S3_v1.0.1_260129; improved driving stability";
...
void md750t_ctrl_task(void *arg) {
    ...
    while(1) {        
        ...
        if (current_read_time - last_loadcell_time > 200) { 
            ...
            float ramp_dn_step = 0.01f;   // 감속 스텝 (가속보다 빠르게 멈춤)
            ...
            if (diff_ch0 > diff_step) {
                // 반응하지 않는 구간 건너뛰기
                if (set_voltage_ch0 > 2.2 && set_voltage_ch0 < 2.5) {
                    set_voltage_ch0 = 2.2f;
                } else if (set_voltage_ch0 > 2.5 && set_voltage_ch0 < 2.8) {
            ...
            if (diff_ch1 > diff_step) {
                // 반응하지 않는 구간 건너뛰기
                if (set_voltage_ch1 > 2.2 && set_voltage_ch1 < 2.5) {
                    set_voltage_ch1 = 2.2f;
                } else if (set_voltage_ch1 > 2.5 && set_voltage_ch1 < 2.8) {
