...
/**
 * @brief 0점 조절 버튼이 눌렸을 때 실행되는 ISR (디바운싱 적용)
 */
static void IRAM_ATTR zero_set_request_isr_handler(void *arg) {
    int64_t current_time = esp_timer_get_time();
    if ((current_time - last_interrupt_time) > (DEBOUNCE_DELAY_MS * 1000)) {    // 50ms 디바운스
...
void pslh080_init(void) {
    ...
    // 0점 조절 버튼 GPIO를 인터럽트 모드로 초기화
    gpio_config_t zero_set_io_conf = {
        .pin_bit_mask = (1ULL << ZERO_SET_BUTTON_GPIO),
        .mode = GPIO_MODE_INPUT,
        .pull_up_en = GPIO_PULLUP_ENABLE,
        .intr_type = GPIO_INTR_NEGEDGE,
    };
    ESP_ERROR_CHECK(gpio_config(&zero_set_io_conf));
    
    // 0점 조절 버튼 인터럽트 핸들러 등록
    ESP_ERROR_CHECK(gpio_isr_handler_add(ZERO_SET_BUTTON_GPIO, zero_set_request_isr_handler, NULL));
